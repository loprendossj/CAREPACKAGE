...
{	
    ...
    void MissionServer()
    {
        ...
        //LootChestsConfig.UpgradeConfig("LootChests_V106.json", config);
		LootChestsConfig.LoadConfig("LootChests_V106.json", config);
        GetDayZGame().SetLootChestsConfig(config);
		
		LCToolsConfig toolsconfig;
		LootChestsToolsConfig.LoadConfig("LootChestsTools_V106.json" , toolsconfig);
		GetDayZGame().SetLootChestsToolsConfig(toolsconfig);
		
		if (GetGame().IsServer()) 
        {
		    g_LootChestsEarlyRun = null;
			GetLootChestsEarlyRun();
			g_LootChestsManager = null;
			GetGame().GetCallQueue( CALL_CATEGORY_GAMEPLAY ).CallLater( RunLCLater, 30000 , false);
	...
	override void InvokeOnConnect(PlayerBase player, PlayerIdentity identity)
	{
		...
		GetRPCManager().SendRPC( "CJ_LootChests", "SyncConfig", new Param1<LCToolsConfig>(toolsconfig) );
	}
	*/
};
